...
class CPD:
    ...
    def hampel_filter(
        ...
        for i, new_point in enumerate(data_stream):
            ...
            if (
                ...
                outlier_indices = [
                    idx
                    for idx, point in enumerate(convinced_data)
                    if np.abs(point - median) > threshold
                ]
                ...
                if outlier_indices:
                    ...
                    max_outlier_idx = max(
                        outlier_indices,
                        key=lambda idx: np.abs(convinced_data[idx] - median),
                    )
                    ...
                    outliers.append(
                        (i - len(convinced_data) + max_outlier_idx, max_outlier_value)
                    )
                    is_outlier = True
                    outlier_info = (
                        i - len(convinced_data) + max_outlier_idx,
                        max_outlier_value,
                    )
                else:
                    is_outlier = False
            ...
            if len(data_history) > start:
                ...
                plt.plot(
                    range(start, len(data_history)),
                    data_history[start : len(data_history)],
                    color=colors[current_color_index],
                )  # 绘制折线的对应部分
    ...
    def merge_to_gif(self, duration=100, loop=0):
        ...
        def extract_number(filename):
            ...
            number_part = filename.split("_")[-1].split(".")[
                0
            ]  # 分割字符串并获取数字部分
        ...
        pic_files = [
            i
            for i in files
            if (i.split(".")[-1]) in ["jpg", "png", "jpeg", "JPG", "PNG", "JPEG"]
        ]
        ...
        try:
            ...
            images[0].save(
                os.path.join(dirpath, output_name),
                save_all=True,
